...
**You MUST complete this before reporting completion to the user.** If `.specify/extensions.yml` doesn't exist or has no `hooks.after_clarify` entries, skip straight to the Completion Report. Otherwise, for each hook enabled (default true) with no `condition` (conditional ones are left to the HookExecutor): convert `.`→`-` in the command name. `optional: false` hooks MUST be executed (emit `EXECUTE_COMMAND: <hyphenated command, same as in /{command}>`, then actually invoke and await it — emitting the block alone doesn't run it) before the Completion Report; `optional: true` hooks are just surfaced as a suggested `/{command}` with its description and any prompt text. If the file fails to parse, tell the user (with the parser error) that hooks weren't checked and continue to the Completion Report.

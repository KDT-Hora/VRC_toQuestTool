...
Report the convergence outcome (`converged` or `tasks_appended`) in-session before listing any hooks, so users can decide whether to run optional follow-up commands. Then apply the same `hooks.after_converge` check described under Pre-Execution Checks above (enabled by default, skip conditional hooks, convert `.`→`-` in command names). `optional: false` hooks MUST be executed and awaited (`EXECUTE_COMMAND: <hyphenated command, same as in /{command}>`); `optional: true` hooks are just surfaced as a suggested `/{command}` with its description and any prompt text. Parse errors: tell the user and continue. Absent file/key: skip silently.

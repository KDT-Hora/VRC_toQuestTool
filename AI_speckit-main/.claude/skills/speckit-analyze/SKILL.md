...
After reporting, apply the same `hooks.after_analyze` check described under Pre-Execution Checks above (enabled by default, skip conditional hooks, convert `.`→`-` in command names). `optional: false` hooks MUST be executed and awaited (`EXECUTE_COMMAND: <hyphenated command, same as in /{command}>`); `optional: true` hooks are just surfaced as a suggested `/{command}` with its description and any prompt text. Parse errors: tell the user and continue. Absent file/key: skip silently.
